...
export default function FootHeatmap({ sensors = {} }) {
  ...
  const p5 = sensors.p5 || (sensors.p2 || sensors.p4 || 0);

  // Normalize 12-bit ADC (0 - 4095)
  const nHeel = Math.min(1.0, p1 / 3800);
  const nForefoot = Math.min(1.0, (p5 || p2 || p4) / 3800);

  const totalForce = p1 + (p5 || p2 || p4);
  const estNewtons = Math.round((totalForce / 8000) * 700);

  // 2 Square FSRs (Matching Physical Hardware)
  const dualFsrNodes = [
    { id: 'FSR 2', name: 'Square Forefoot FSR', role: 'Propulsion & Metatarsal Loading', raw: (p5 || p2 || p4), norm: nForefoot, x: 65, y: 95, w: 70, h: 65, rx: 12 },
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Calibrated for dual square FSR channels (Calcaneus Heel + Metatarsal Ball)
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center my-auto">
        ...
        <div className="md:col-span-6 flex justify-center relative py-2">
          ...
          >
            ...
            {/* 2x Square FSR Sensors (Hardware Matched) */}
            {dualFsrNodes.map((s) => {
